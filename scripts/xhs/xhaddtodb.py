...
def add_note( note_id, title):
    try:
        sql = f'replace INTO "xhs_note" ("note_id", "title") VALUES ( "{note_id}", "{title}");'
        cur = con.cursor()
        cur.execute(sql)
        con.commit()
...
if __name__ == '__main__':
    ...
    for i in range(1, 3000):
        try:
            data = client.get_note_by_keyword(keyword=keyword, sort=SearchSortType.LATEST, page=i)
